...
export default class helperDataPathing {
    ...
    static getValueAtPath = (argArrayPath, arg) => {
        ...
        let item = arg;
        let itemIntIndexForArgArrayPath = -1;
        const intLength = argArrayPath.length;
        while (++itemIntIndexForArgArrayPath < intLength) {
            const itemKey = argArrayPath[itemIntIndexForArgArrayPath];
            switch (_helperEnumDataTypes.getEnumDataType(item)) {
                case _helperEnumDataTypes.fieldArray:
                    //
                ...
                case _helperEnumDataTypes.fieldObject:
                    ...
                    if (Object.keys(item).includes(itemKeyString)) {
                        item = item[itemKeyString];
                    } else {
                        return helperDataPathing._getErrorBecausePathFailed(
                            argArrayPath,
                            itemIntIndexForArgArrayPath,
    ...
    static _getErrorBecausePathFailed(
        ...
        const arrayOfStrings = [
            ...
            `Path = ${argArrayPath}`,
            `Array path that exists = ${argArrayPath.slice(
                0,
                argIntIndexOfFailure
            )}`,
            ...
            `Is root object defined? ${
                argObjectRoot !== undefined && argObjectRoot !== null
                    ? "yes"
                    : "no"
            }`,
        ];
        switch (_helperEnumDataTypes.getEnumDataType(argObjectOfFailure)) {
            case _helperEnumDataTypes.fieldArray:
                arrayOfStrings.push(
                    `Array object's size = ${argObjectOfFailure.length}`
                );
                break;
            case _helperEnumDataTypes.fieldObject:
                arrayOfStrings.push(
                    `Object's keys = ${Object.keys(argObjectOfFailure).sort()}`
                );
                break;
            default:
        ...
        return helperErrors.raiseError(
            Error(
                helperDataPathing._getStringByCombiningArray(
                    arrayOfStrings,
                    "\n"
                )
            )
